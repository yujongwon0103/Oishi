...
public class MyFavoriteListFragment extends ListFragment {
    private ArrayList<String> favorite_store_list = new ArrayList<>();
    private FoodBrandAdapter adapter;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        inquiries_favorite_list();

        String url = "http://ykh3587.dothome.co.kr/???.php";

        ContentValues values = new ContentValues();
        for(int i = 0 ; i < favorite_store_list.size() ; i++) {
            values.put("favorite_store_"+i, favorite_store_list.get(i));
        }
    ...
    private void inquiries_favorite_list(){
        try {
            String uriString = "content://com.example.sql_db/User_Favorite";
            Uri uri = new Uri.Builder().build().parse(uriString);

            String[] columns = new String[] {"favorite_store"};
            if(getActivity() != null) {
                Cursor cursor = getActivity().getApplicationContext().getContentResolver().query(uri, columns, null, null, "_id ASC");

                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        String favorite_store = cursor.getString(cursor.getColumnIndex(columns[0]));
                        favorite_store_list.add(favorite_store);
                    }
                    cursor.close();
                }
            }
