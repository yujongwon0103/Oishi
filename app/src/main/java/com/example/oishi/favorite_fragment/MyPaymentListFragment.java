...
public class MyPaymentListFragment extends ListFragment {
    private ArrayList<String> payment_store_list = new ArrayList<>();
    private FoodBrandAdapter adapter;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        inquiries_payment_list();

        String url = "http://ykh3587.dothome.co.kr/???.php";

        ContentValues values = new ContentValues();
        for(int i = 0 ; i < payment_store_list.size() ; i++) {
            values.put("payment_store_"+i, payment_store_list.get(i));
        }
    ...
    private void inquiries_payment_list(){
        try {
            String uriString = "content://com.example.sql_db/User_Payment";
            Uri uri = new Uri.Builder().build().parse(uriString);

            String[] columns = new String[] {"immediate_payment"};
            if(getActivity() != null) {
                Cursor cursor = getActivity().getApplicationContext().getContentResolver().query(uri, columns, null, null, "_id ASC");

                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        String immediate_payment = cursor.getString(cursor.getColumnIndex(columns[0]));
                        payment_store_list.add(immediate_payment);
                    }
                    cursor.close();
                }
            }
